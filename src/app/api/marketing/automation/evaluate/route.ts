...
import { evaluateAutomationRules } from '@/lib/marketing/automation';
...
async function runEvaluate(request: NextRequest, body: Record<string, unknown>) {
  ...
  try {
    ...
    console.error('[marketing/automation/evaluate] error:', error);
    return jsonError(error instanceof Error ? error.message : 'Unable to evaluate automation rules', 500);
  }
}

export async function POST(request: NextRequest) {
  return runEvaluate(request, await request.json().catch(() => ({})));
}

export async function GET(request: NextRequest) {
  if (!authorizedCron(request)) return jsonError('Unauthorized', 401);
  return runEvaluate(request, {
    campaign_id: request.nextUrl.searchParams.get('campaign_id'),
    execute: request.nextUrl.searchParams.get('execute') === 'true',
  });
}
